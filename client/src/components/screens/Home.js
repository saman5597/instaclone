                ...
                data.length ? data.map(item => {
                    return (
                        <div key={item._id} className="card home-card">
                            <h5 style={{ padding: "6px" }}><img className="profile-pic-small" alt={item.postedBy.fullName} src={item.postedBy.profilePic} /> <Link to={item.postedBy._id !== profile._id ? `/profile/${item.postedBy._id}` : "/profile"}>{item.postedBy.fullName}</Link>
                                {
